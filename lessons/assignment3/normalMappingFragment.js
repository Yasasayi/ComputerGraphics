...
/* (Assign3) normal map texture등 필요한 uniform들을 추가 선언한다. */
uniform sampler2D u_normalMap;
uniform float u_normalMappingIntensity;
uniform bool u_normalMapFlag; // u_normalMap이 존재하는지 확인

in vec2 v_texcoord; 
in vec3 v_normal; 
in vec3 v_worldPosition; 
in mat3 v_TBN; // (Assign3) Tangent Space Transformation matrix (vertex shader 참고)
...
{
  /* (Assign3) normal map을 사용하도록 calculateLight를 수정한다.
     (HINT)동일한 셰이더를 노멀맵 효과를 적용하는 물체와 아닌 물체에 동일하게 사용하기 때문에
     분기를 구현해야만 한다.
     노멀맵 효과 계산 중, 필요한 경우 위 ToTangenSpaceVector() 함수를 사용한다.*/

  if (u_normalMapFlag)
  {
    vec3 normal = normalize(u_normalMappingIntensity * texture(u_normalMap, v_texcoord).xyz - 1.0);
    // how to re-calculate shadow factor..?

    //ambient
    vec3 lightAmbient = light.lightColor * light.ambientIntensity;
    vec3 lightDir = normalize(-ToTangenSpaceVector(direction));
    float diffuseFactor = max(dot(normal, lightDir), 0.0);
    vec3 lightDiffuse = light.lightColor * light.diffuseIntensity * diffuseFactor;
    
    //specular
    vec3 vVec = normalize(ToTangenSpaceVector(u_eyePosition - v_worldPosition)); 
